...
- Loads the *first-pass* dataset (apps_research.json as emitted by seed.py).
- Applies the agent-verified ground truth for the 40-app sample (2 waves of
  parallel research sub-agents) plus one manual doc adjudication (Waterfall).
- Computes first-pass vs verified accuracy, then writes the corrected dataset.
- Emits verification.json (sample detail + metrics) and aggregates.json
  (counts used by the HTML report).
...
# ---- Verified ground truth for the 40-app sample -------------------------
...
def main():
    fp_path = os.path.join(HERE, "apps_research.json")
    with open(fp_path, encoding="utf-8") as f:
        firstpass = json.load(f)
    by_id = {r["id"]: r for r in firstpass}
    ids = list(VERIFIED.keys())

    # ---- compute first-pass vs verified metrics ----
    sample = []
    ...
    # 'free self-serve' discovery (false negatives for buildability)
    ...
    metrics = {
        ...
        "corrected_blocked_precision": 1.0,
        "corrected_blocked_recall": 1.0,
    }
    ...
    for r in firstpass:
        if r["id"] in VERIFIED:
            r = dict(r)
            for k, v in VERIFIED[r["id"]].items():
                r[k] = v
        corrected.append(r)
    ...
    verification = {
        "waves": 2, "method": "Parallel research sub-agents (2 waves x 4 agents x 5 apps = 40 apps) + manual doc adjudication (Waterfall).",
        "metrics": metrics,
        "sample": sample,
    }
    ...
    # ---- aggregates over corrected 100 ----
    agg = aggregate(corrected)
    with open(os.path.join(HERE, "aggregates.json"), "w", encoding="utf-8") as f:
        json.dump(agg, f, indent=2, ensure_ascii=False)

    print("First-pass blocked:", len(fp_blocked), "Verified blocked:", len(ve_blocked))
    print("First-pass blocked precision:", round(firstpass_blocked_precision,3))
    print("Self-serve exact match:", round(ss_match/n,3), "Verdict exact match:", round(ve_match/n,3), "MCP exact match:", round(mc_match/n,3))
    print("Apps changed by verification:", sum(1 for s in sample if s['changed']), "/", n)
